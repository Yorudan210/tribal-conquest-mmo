...
function villageMapIconSvg(kind, level, hasWall){
  ...
  const hut = (x,s,chimney)=>{
    const W=15, D=12, H=10, RH=9;
    const F={x:-0.435*(W-D), y:0.25*(W+D)};
    const R={x:F.x+W*0.87, y:F.y-W*0.5}, L={x:F.x-D*0.87, y:F.y-D*0.5};
    const K={x:R.x-D*0.87, y:R.y-D*0.5};
    const raise=(p,dy)=>({x:p.x, y:p.y-dy});
    const Ft=raise(F,H), Rt=raise(R,H), Lt=raise(L,H), Kt=raise(K,H);
    ...
    const d0=lerp(F,R,.18), d1=lerp(F,R,.52), dh=H*0.55;
    const w0=lerp(F,L,.46), w1=lerp(F,L,.78), wy0=H*.4, wy1=H*.76;
    const cx=3.6, cy=apex.y+2.5;
