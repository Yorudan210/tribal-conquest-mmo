...
function villageMapIconSvg(kind, level, hasWall){
  ...
  // pensée pour l'angle de caméra des grandes scènes de VillageActionModal, pas pour une icône
  // regardée droit dessus sur la carte). Pas de mur/porte/fenêtre visibles d'en haut : seuls le
  // toit et la cheminée dépassent.
  const hut = (cx,cy,s,rot,chimney)=>{
    const W=13, D=10, hw=W/2, hd=D/2;
    const chx=hw*0.42, chy=-hd*0.32;
    return `<g transform="translate(${cx},${cy}) rotate(${rot}) scale(${s})">
      <rect x="${-hw}" y="${-hd}" width="${W}" height="${D}" fill="${roofTop}"/>
      <rect x="${-hw}" y="0" width="${W}" height="${hd}" fill="${roofBot}"/>
      <rect x="${-hw}" y="${-hd}" width="${W}" height="${D}" fill="none" stroke="${doorStroke}" stroke-width="1"/>
      <line x1="${-hw}" y1="0" x2="${hw}" y2="0" stroke="${doorStroke}" stroke-width="0.7" opacity="0.55"/>
      ${chimney ? `<rect x="${(chx-1.3).toFixed(1)}" y="${(chy-1.3).toFixed(1)}" width="2.6" height="2.6" fill="${doorStroke}"/><circle cx="${(chx-1.3).toFixed(1)}" cy="${(chy-2.6).toFixed(1)}" r="1.6" fill="#d8d2c2" opacity="0.4"/><circle cx="${(chx-2.3).toFixed(1)}" cy="${(chy-4.1).toFixed(1)}" r="2" fill="#d8d2c2" opacity="0.24"/>` : ""}
    </g>`;
  };
  // Regroupement en petit amas (décalés en x ET en y, avec une légère rotation chacune) plutôt
  // qu'en simple rangée horizontale : une rangée se lit comme une façade vue de côté, un amas se
  // lit comme un hameau vu du ciel.
  const huts = level<=0
    ? hut(0,2,1,0,true)
    : level===1
    ? hut(-8,-2,0.85,-10,false)+hut(7,5,0.95,8,true)
    : hut(-1,-7,0.85,-4,true)+hut(-11,5,0.78,-14,false)+hut(10,4,0.82,12,false);
  ...
  const groundRx = level<=0 ? 14 : level===1 ? 19 : 23;
  const groundRy = level<=0 ? 10 : level===1 ? 13 : 15;
  const groundCy = level<=0 ? 3 : level===1 ? 2 : 1;
  const ground = `<ellipse cx="0" cy="${groundCy}" rx="${groundRx}" ry="${groundRy}" fill="${groundCol}" opacity="0.55"/>`;
  // Élément central du hameau/village (niveau >= 1), vu du dessus lui aussi : cercle de pierres
  // avec la flamme au centre pour un feu de camp, margelle circulaire avec son puits sombre au
  // centre pour un village -- rien pour l'Armée Noire, dont l'ambiance sombre + la lueur rouge du
  // pin suffisent déjà à la distinguer sans surcharger l'icône.
  const centerY = level===1 ? 9 : 11;
  const centerpiece = level>=1 && !black
    ? (campKind
        ? `<g transform="translate(0,${centerY})"><circle r="3.6" fill="none" stroke="${doorStroke}" stroke-width="1.3"/><circle r="2" fill="#e0862c" opacity="0.85"/><circle r="1" fill="#f5c25a"/></g>`
        : `<g transform="translate(0,${centerY})"><circle r="4.2" fill="none" stroke="#6b5533" stroke-width="1.3"/><circle r="3.1" fill="#241c10" stroke="${doorStroke}" stroke-width="0.6"/></g>`)
    : "";
  const palisade = hasWall
    ? `<ellipse cx="0" cy="${groundCy}" rx="${groundRx+4}" ry="${groundRy+4}" fill="none" stroke="${black?'#3a1414':legendary?'#6b4a12':convoy?'#6b4a08':raiders?'#5a2f18':bandits?'#453552':'#5a4326'}" stroke-width="2.2" stroke-dasharray="3.5 3" opacity="0.85"/>`
    : "";
 ...
 * déjà utilisée par villageMapIconSvg() pour kind==="legendary" (mêmes teintes de toit que le pin
 * sur la carte : voir roofTop/roofBot/doorStroke ci-dessus, et le glyphe 👑 doré #f2c94c déjà
 * utilisé pour cette faction dans mapRender.js/styles.css), pour que ce grand rendu et le
...
const LC_ISO_X = 0.87, LC_ISO_Y = 0.5;
const lcIsoRight = n => ({x:n*LC_ISO_X, y:-n*LC_ISO_Y});
const lcIsoLeft  = n => ({x:-n*LC_ISO_X, y:-n*LC_ISO_Y});
const lcAdd = (a,b) => ({x:a.x+b.x, y:a.y+b.y});
const lcRaise = (p,dy) => ({x:p.x, y:p.y-dy});
const lcLerp = (a,b,t) => ({x:a.x+(b.x-a.x)*t, y:a.y+(b.y-a.y)*t});
...
function lcPt(cx,cy,rx,ry,deg){ const a=deg*Math.PI/180; return {x:cx+rx*Math.cos(a), y:cy+ry*Math.sin(a)}; }
function lcGateAngles(gapDeg){ return { start:90+gapDeg/2, end:90+gapDeg/2+(360-gapDeg) }; }
...
function lcBuilding(x,y,s,roofLit,roofShade,wallLit,wallShade,doorCol,opts){
  ...
  const F={x:fx,y:fy}, R=lcAdd(F,lcIsoRight(W)), L=lcAdd(F,lcIsoLeft(D)), K=lcAdd(R,lcIsoLeft(D));
  const Ft=lcRaise(F,H), Rt=lcRaise(R,H), Lt=lcRaise(L,H), Kt=lcRaise(K,H);
  let out = "";
  out += lcPoly([F,R,Rt,Ft], wallLit, edge);
  out += lcPoly([F,L,Lt,Ft], wallShade, edge);
  if(opts.door!==false){
    const d0=lcLerp(F,R,.16), d1=lcLerp(F,R,.42), dh=H*0.52;
    out += lcPoly([d0,d1,lcRaise(d1,dh),lcRaise(d0,dh)], doorCol, edge);
  }
  if(opts.window!==false){
    const w0=lcLerp(F,L,.5), w1=lcLerp(F,L,.76), wy0=H*.4, wy1=H*.72;
  ...
  if(RH>0){
    ...
    if(opts.banner){
      ...
      out += lcPoly([{x:bx,y:by-13*s},{x:bx+10*s,y:by-8*s},{x:bx,y:by-4*s}], opts.banner);
    }
    return { svg: out, anchor: Ft };
  }
  out += lcPoly([Ft,Rt,Kt,Lt], wallShade, edge);
  return { svg: out, anchor: Ft };
}
function lcHut(x,y,s,roofA,roofB,wallA,wallB,doorCol){
  return lcBuilding(x,y,s,roofA,roofB,wallA,wallB,doorCol,{w:23,d:19,h:15,roofH:15}).svg;
}
function lcKeep(x,y,s,roofA,roofB,wallA,wallB,bannerCol){
  const base = lcBuilding(x,y,s,roofA,roofB,wallA,wallB,"#1c130a",{w:32,d:25,h:16,roofH:0,window:false});
  const upperS = s*0.6;
...
function lcTower(x,y,s,wallCol,wallEdge,roofCol,bannerCol){
  return lcBuilding(x,y,s,roofCol,roofCol,wallCol,wallEdge,"#160f08",
    {w:19,d:17,h:34,roofH:16,door:false,window:true,windowCol:"#ffd54a",edge:wallEdge,banner:bannerCol}).svg;
}
...
function lcGateDoor(bs,be,h,ironCol){
  ...
  const woodA="#3a280f", woodB="#1c1206";
...
function lcWallToppers(cx,topCy,rx,ry,gapDeg,count,colBase,colShade,colRim){
  ...
  for(let i=0;i<count;i++){
    ...
    const bo0 = lcPt(cx,topCy,rx,ry,a0), bo1 = lcPt(cx,topCy,rx,ry,a1);
    const to0 = lcRaise(bo0,mh), to1 = lcRaise(bo1,mh);
    const ti0 = lcPt(cx,topCy-mh,rx-depth,ry-depth,a0), ti1 = lcPt(cx,topCy-mh,rx-depth,ry-depth,a1);
...
function lcWallBand(cx,cy,rx,ry,gapDeg,h,colBase,colShade,colRim,gateBanner){
  ...
  const ts = lcPt(cx,topCy,rx,ry,g.start), te = lcPt(cx,topCy,rx,ry,g.end);
  const bs = lcPt(cx,cy,rx,ry,g.start), be = lcPt(cx,cy,rx,ry,g.end);
  const its = lcPt(cx,topCy,irx,iry,g.start), ite = lcPt(cx,topCy,irx,iry,g.end);
  ...
  for(let k=1;k<=2;k++){
    ...
    const cA = lcPt(cx,yy,rx,ry,g.start), cB = lcPt(cx,yy,rx,ry,g.end);
...
function legendaryCampSceneSvg(){
  // Mêmes teintes de toit que villageMapIconSvg(kind="legendary") : roofTop/roofBot -- voir plus
  // haut dans ce fichier.
  const wallBase="#4a3510", wallShade="#160e02", gold="#f2c94c", roofA="#9a2210", roofB="#380803";
  const wallA="#2a1f10", wallB="#160e02", groundA="#241a06", groundB="#120c02";
  const CX=160, CY=150;
