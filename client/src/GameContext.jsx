...
export function GameProvider({ children }){
  ...
  const goToVillageOnMap = useCallback((villageId) => {
    ...
    if(!t) return;
    setActiveTab("map");
    setMapView(prev => ({ cx:t.x, cy:t.y, ppf:(prev&&prev.ppf)||26 }));
      ...
      for(const r of snap.reports){
        if(r.kind==="scout" && r.targetId!=null){
          ...
          if(!ex || r.time > ex.time){
            ...
            next[r.targetId] = { resources:r.resources, troops:r.troops, wallLevel:r.wallLevel, loyalty:r.loyalty, time:r.time, isPlayer:r.isPlayer };
          }
        }
      }
      return next;
      ...
      if(newCount>0 && newCount<=snap.reports.length){
        snap.reports.slice(0, newCount).forEach(r=>{
          if(r.kind==="defense"){
            ...
            setBanner({ id:Date.now()+Math.random(), type:"raid",
  ...
  const connectWs = useCallback(() => {
    ...
    let socket;
    try{ socket = new WebSocket(wsUrl()); }catch(e){ scheduleWsReconnect(); return; }
    wsRef.current = socket;
    socket.onopen = () => {
      wsReconnectDelay.current = 1000;
      try{ socket.send(JSON.stringify({type:"auth", token:authTokenRef.current})); }catch(e){}
    };
    socket.onmessage = (ev) => {
      let msg;
    ...
    socket.onclose = () => { if(wsRef.current===socket) wsRef.current=null; scheduleWsReconnect(); };
  ...
  const disconnectWs = useCallback(() => {
    ...
    wsReconnectDelay.current = 1000;
    if(wsRef.current){ const s=wsRef.current; wsRef.current=null; try{ s.onclose=null; s.close(); }catch(e){} }
  }, []);
  ...
  const login = useCallback(async (u,p) => {
    ...
    try{
      ...
      enterGame(data);
    }catch(err){ setAuthError(err.message); throw err; }
  }, [enterGame]);
  ...
  const logout = useCallback((silent) => {
    stopPolling();
    disconnectWs();
    setAuthToken(null); setUsername(null); setSnapshot(null);
    setScoutIntel({}); setSparkleUntil({}); setBanner(null); setTutorialOpen(false);
    authTokenRef.current = null; usernameRef.current = null; snapshotRef.current = null;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if(!savedToken || !savedUser){ setResuming(false); return; }
      authTokenRef.current = savedToken; usernameRef.current = savedUser;
      try{
        ...
        if(cancelled) return;
        authTokenRef.current = null; usernameRef.current = null;
        localStorage.removeItem("ct_token"); localStorage.removeItem("ct_username");
      } finally {
