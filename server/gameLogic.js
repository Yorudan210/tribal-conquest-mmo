...
/* ------------------------------------------------------------------------ */
/*  Évènements PvE ROTATIFS (déclenchés par un admin, un seul actif à la      */
/*  fois) : une vague de campements PNJ hostiles, distincts des villages      */
/*  barbares normaux par un champ "faction" et une couleur dédiée sur la      */
/*  carte. Un camp reste un village owner:"barbarian" ordinaire (même         */
/*  combat, même conquête au Noble, même conversion "semi-vivante" selon      */
/*  son tier) pour réutiliser tel quel tout le moteur déjà éprouvé -- seuls   */
/*  la génération, l'affichage et le suivi de succès sont spécifiques au      */
/*  thème actif (voir EVENT_FACTIONS, shared/gameData.js).                    */
/*                                                                            */
/*  Généralisé à partir de l'évènement de lancement "L'Armée Noire" (5 rangs, */
/*  tier 0 à 4 comme les barbares, nettement plus forts et plus riches) :     */
/*  le thème "blackArmy" d'EVENT_FACTIONS reproduit EXACTEMENT ses anciennes  */
/*  constantes -- aucun changement de comportement pour ce thème. Le champ    */
/*  "db.blackArmyEvent" garde également son nom d'origine (déjà utilisé en    */
/*  production) ; un champ "key" supplémentaire (absent = "blackArmy") dit    */
/*  quel thème est actif, sans nécessiter de migration de data/db.json.       */
/* ------------------------------------------------------------------------ */
...
function findFreeEventCoord(db){
...
function pickEventTier(tierWeights){
  const r = Math.random();
  let acc = 0;
  for(let i=0;i<tierWeights.length;i++){ acc += tierWeights[i]; if(r<acc) return i; }
  return tierWeights.length-1;
}

/* Fait apparaître jusqu'à "count" campements du thème d'évènement "cfg" (une entrée d'EVENT_FACTIONS,
   voir shared/gameData.js) -- peut en placer moins si la carte est pleine. Retourne la liste des
   identifiants de village créés. Généralisation de l'ancien spawnBlackArmy(db,count), qui utilisait
   ces mêmes formules mais avec les constantes de l'Armée Noire codées en dur. */
function spawnEventFaction(db, cfg, count){
  const ids = [];
  for(let i=0;i<count;i++){
    const coord = findFreeEventCoord(db);
    if(!coord) break;
    const tier = pickEventTier(cfg.tierWeights);
    ...
    db.villages[id] = {
      ...
      name: cfg.namePool[Math.floor(Math.random()*cfg.namePool.length)],
      ...
      resCap: Math.round((600+tier*500)*cfg.resCapMult),
      troops, wallLevel: Math.min(cfg.wallMax, cfg.wallBase+tier*cfg.wallPerTier), hideLevel: 0,
      loyalty: 100, aggro: {},
      // Comme l'Armée Noire à l'origine : si le thème le prévoit (guaranteedResourceBonus), le
      // gisement bonus n'est plus une chance sur huit (voir rollResourceBonus, store.js) mais garanti
      // -- une récompense sûre pour les joueurs équipés pour aller jusqu'au bout de la conquête.
      resourceBonus: cfg.guaranteedResourceBonus ? { res: bonusRes, pct: cfg.resourceBonusPct } : null
    };
    ids.push(id);
  }
  return ids;
...
/* Retire du monde tous les campements du thème actif pas encore conquis (ceux encore
   ...
   automatique (runTick). Généralisation de l'ancien endBlackArmyEvent -- nom de fonction conservé
   (appelé par runTick) mais le retrait cible désormais le thème réellement actif, pas toujours
   "blackArmy" ; les événements déjà enregistrés sans champ "key" (avant ce système générique)
   continuent de retirer des campements "blackArmy", comme avant. */
function endBlackArmyEvent(db, announce){
  if(!db.blackArmyEvent) return { ok:true, removed:0 };
  const key = db.blackArmyEvent.key || "blackArmy";
  const cfg = GameData.EVENT_FACTIONS[key] || GameData.EVENT_FACTIONS.blackArmy;
  let removed = 0;
  for(const id in db.villages){
    const v = db.villages[id];
    if(v.owner==="barbarian" && v.faction===key){ delete db.villages[id]; removed++; }
  }
  db.blackArmyEvent.active = false;
  if(announce!==false){
    adminAnnounce(db, "Système", cfg.icon+" "+cfg.name+" se retire du monde. Les "+removed+" campements restants disparaissent -- merci à ceux qui ont pris part à l'évènement !");
  }
  return { ok:true, removed };
}

/* Lance un nouvel évènement rotatif pour le thème "key" (une clé d'EVENT_FACTIONS). Généralisation de
   l'ancien adminStartBlackArmy(db,count,minutes) -- toujours exporté ci-dessous comme un alias fin qui
   appelle celle-ci avec key="blackArmy", pour ne rien changer côté route /api/admin/blackarmy/start. */
function adminStartEvent(db, key, count, minutes){
  const cfg = GameData.EVENT_FACTIONS && GameData.EVENT_FACTIONS[key];
  if(!cfg) return { error:"Thème d'évènement inconnu." };
  count = Math.floor(Number(count));
  minutes = Math.floor(Number(minutes));
  if(db.blackArmyEvent && db.blackArmyEvent.active) return { error:"Un évènement est déjà en cours -- arrêtez-le d'abord pour en relancer un." };
  if(!Number.isFinite(count) || count<cfg.minCount || count>cfg.maxCount) return { error:"Nombre de campements invalide (doit être entre "+cfg.minCount+" et "+cfg.maxCount+")." };
  if(!Number.isFinite(minutes) || minutes<cfg.minMinutes || minutes>cfg.maxMinutes) return { error:"Durée invalide (doit être entre "+cfg.minMinutes+" et "+cfg.maxMinutes+" minutes)." };
  const ids = spawnEventFaction(db, cfg, count);
  if(!ids.length) return { error:"Impossible de placer le moindre campement (carte pleine)." };
  db.blackArmyEvent = { key: cfg.key, active:true, startAt: now(), endAt: now()+minutes*60, totalSpawned: ids.length, defeatedCount: 0 };
  adminAnnounce(db, "Système",
    cfg.icon+" "+cfg.name.toUpperCase()+" ENVAHIT LE MONDE ! "+ids.length+" campements viennent d'apparaître sur la carte. "+cfg.desc+" Consultez l'aide pour le détail des règles. Retrait des campements restants dans "+minutes+" minutes."
  );
  return { ok:true, spawned: ids.length };
}

function adminStopEvent(db){
  if(!db.blackArmyEvent || !db.blackArmyEvent.active) return { error:"Aucun évènement en cours." };
  return endBlackArmyEvent(db, true);
}

// Alias conservés pour compatibilité avec les routes /api/admin/blackarmy/* existantes (voir
// server.js) -- appellent simplement la fonction générique avec key="blackArmy".
function adminStartBlackArmy(db, count, minutes){ return adminStartEvent(db, "blackArmy", count, minutes); }
function adminStopBlackArmy(db){ return adminStopEvent(db); }

/* Vue publique de l'évènement en cours (ou du dernier passé), envoyée dans chaque instantané à tous
   les joueurs -- pas seulement aux admins -- pour afficher la bannière "Encore XhYm". Inclut désormais
   le thème actif (key/name/icon) pour que le client puisse afficher le bon nom/icône sans avoir à
   deviner -- avant ce système générique, "L'Armée Noire" était codé en dur côté client. */
function publicBlackArmyEvent(db){
  ...
  return {
    active, key, name: cfg.name, icon: cfg.icon,
...
function findFreeCoordNearVillage(db, base, radius){
  ...
  return findFreeEventCoord(db); // dernier recours : coordonnée libre n'importe où sur la carte
}
...
function resolveAttack(db, m){
  ...
  if(winner==="attacker"){
    ...
    // Évènement rotatif (voir EVENT_FACTIONS, shared/gameData.js) : chaque victoire contre un campement
    // dont la faction correspond à un thème d'évènement crédite le succès dédié de ce thème
    // (achievementStat) et, si CET évènement précis est celui actuellement/dernièrement actif (même
    // clé), incrémente son compteur de victoires affiché dans le panneau Admin. Généralisation de
    // l'ancien traitement, qui ne connaissait que "blackArmy" -- le thème "blackArmy" garde exactement
    // le même comportement (achievementStat:"blackArmyDefeated").
    const eventCfg = GameData.EVENT_FACTIONS && GameData.EVENT_FACTIONS[target.faction];
    if(eventCfg){
      bumpStat(db, m.attackerUsername, eventCfg.achievementStat, 1);
      if(db.blackArmyEvent && (db.blackArmyEvent.key||"blackArmy")===target.faction){
        db.blackArmyEvent.defeatedCount = (db.blackArmyEvent.defeatedCount||0)+1;
      }
    }
...
const EMPTY_STATS = ()=>({ totalLoot:0, unitsKilled:0, attacksWon:0, supportsSent:0, marketTrades:0, wallLevelsDestroyed:0, blackArmyDefeated:0, legendaryDefeated:0, goldenConvoyDefeated:0, opponents:[] });
...
function computeAchievements(db, username){
  ...
  const values = {
    ...
    goldenConvoyDefeated: stats.goldenConvoyDefeated||0
  };
...
module.exports = {
  ...
  adminStartBlackArmy, adminStopBlackArmy, adminStartEvent, adminStopEvent, adminSeedPermanentFactions,
