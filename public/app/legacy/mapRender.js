...
export function guildRelationFor(snapshot, guildId) {
  ...
  return {
    type: rel.type,
    cls,
    label: DIPLOMACY_LABEL[rel.type]
  };
}
...
export function missionMarkerPos(snapshot, now, m) {
  ...
  return {
    wx: fromX + (toX - fromX) * frac,
    wy: fromY + (toY - fromY) * frac,
    returning
  };
}
export function worldMissionMarkerPos(snapshot, now, m) {
  const from = findAnyVillage(snapshot, m.sourceVillageId),
    to = findAnyVillage(snapshot, m.targetId);
  if (!from || !to) return null;
  const frac = clamp((now - m.departAt) / Math.max(1, m.travel), 0, 1);
  return {
    wx: from.x + (to.x - from.x) * frac,
    wy: from.y + (to.y - from.y) * frac,
    returning: false
  };
}
export function mapMarkerHTML(m, pos, minX, minY, ppf, foreign) {
  const left = (pos.wx - minX) * ppf,
    top = (pos.wy - minY) * ppf;
...
export function buildMapMarkup(snapshot, username, mapView, selectedVillage, now, viewport) {
  ...
  const xs = others.map(t => t.x).concat(v.x),
    ys = others.map(t => t.y).concat(v.y);
  const minX = Math.min(...xs) - 3,
    maxX = Math.max(...xs) + 3;
  const minY = Math.min(...ys) - 3,
    maxY = Math.max(...ys) + 3;
  const worldW = (maxX - minX) * ppf,
    worldH = (maxY - minY) * ppf;
  ...
  const margin = 6;
  const viewMinX = mv.cx - viewW / 2 / ppf - margin,
    viewMaxX = mv.cx + viewW / 2 / ppf + margin;
  const viewMinY = mv.cy - viewH / 2 / ppf - margin,
    viewMaxY = mv.cy + viewH / 2 / ppf + margin;
  const visibleOthers = others.filter(t => t.x >= viewMinX && t.x <= viewMaxX && t.y >= viewMinY && t.y <= viewMaxY);
  const pins = visibleOthers.map(t => {
    const left = (t.x - minX) * ppf,
      top = (t.y - minY) * ppf;
  ...
  const playerLeft = (v.x - minX) * ppf,
    playerTop = (v.y - minY) * ppf;
